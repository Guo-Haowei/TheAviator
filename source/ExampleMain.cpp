...
class BoxApp : public D3DApp {
   ...
   private:
    ...
    std::unique_ptr<UploadBuffer<PerDrawConstant>> mPerDrawCB = nullptr;
    std::unique_ptr<UploadBuffer<PerFrameConstant>> mPerFrameCB = nullptr;
...
void BoxApp::Update()
{
    ...
    mPerFrameCB->CopyData( 0, perFrameCB );

    PerDrawConstant perDrawCB;
    perDrawCB.W = mat4( 1 );
}
...
void BoxApp::BuildDescriptorHeaps()
{
    ...
    DX_CALL( mDevice->CreateDescriptorHeap( &cbvHeapDesc,
                                            IID_PPV_ARGS( &mCbvHeap ) ) );
}

void BoxApp::BuildConstantBuffers()
{
    D3D12_GPU_VIRTUAL_ADDRESS cbAddress = mPerFrameCB->Resource()->GetGPUVirtualAddress();
    {
        mPerFrameCB = std::make_unique<UploadBuffer<PerFrameConstant>>( mDevice.Get(), 1, true );

        const UINT frameCBSizeInByte = d3dUtil::CalcConstantBufferByteSize( sizeof( PerFrameConstant ) );

        cbAddress += frameCBSizeInByte;

        D3D12_CONSTANT_BUFFER_VIEW_DESC cbvDesc;
        cbvDesc.BufferLocation = cbAddress;
        cbvDesc.SizeInBytes = frameCBSizeInByte;
    ...
    {
        mPerDrawCB = std::make_unique<UploadBuffer<PerDrawConstant>>( mDevice.Get(), 1, true );

        const UINT objCBSizeInByte = d3dUtil::CalcConstantBufferByteSize( sizeof( PerDrawConstant ) );
        cbAddress += objCBSizeInByte;

        D3D12_CONSTANT_BUFFER_VIEW_DESC cbvDesc;
        cbvDesc.BufferLocation = cbAddress;
        cbvDesc.SizeInBytes = objCBSizeInByte;
...
void BoxApp::BuildRootSignature()
{
    ...
    DX_CALL( mDevice->CreateRootSignature(
        0,
