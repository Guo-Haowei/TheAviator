...
DynamicEntity::DynamicEntity(EntityType type,
                             RawModel* model,
                             glm::vec3 position,
                             glm::vec3 color,
                             float scale,
...
DynamicEntity::~DynamicEntity()
{
    if(type)
    {
        ...
        ParticleHolder::theOne().spawnParticles(
            position, density, color, (float)density * scale.x / 15.0f);
    }
}
...
void DynamicEntity::addEntity(DynamicEntity* entity)
{
    ...
    if(dynamicEntities.find(key) == dynamicEntities.end())
        dynamicEntities.insert(
            std::pair<RawModel*, vector<DynamicEntity*>>(key, vector<DynamicEntity*>()));
