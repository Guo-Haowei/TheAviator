...
ObstacleHolder::ObstacleHolder()
    : lastSpawnDistance(offscreenLeft)
{ }
...
void ObstacleHolder::spawn(float distance)
{
    if(distance >= lastSpawnDistance + miniumDist_O)
    {
        ...
        glm::vec3 position(
            h * glm::sin(offscreenLeft), h * glm::cos(offscreenLeft) - SEA::RADIUS, 0.0f);
        float scale = 3.0f;
        DynamicEntity* obstacle =
            new DynamicEntity(OBSTACLE, Geometry::sphere, position, obstacleColor, scale);
...
void ObstacleHolder::update()
{
    ...
    for(auto& obstatcle : obstatcles)
    {
        ...
        obstatcle->changeRotation(
            glm::vec3(0.0f, 0.0f, 1.0f), GAME::SPEED, glm::vec3(0.0f, -SEA::RADIUS, 0.0f));
    }
}
