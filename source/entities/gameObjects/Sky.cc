...
void Sky::createCloud(float angle)
{
    ...
    {
        glm::vec3 position(
            (float)i * 5.0f * cloudScale, Maths::rand(0.0f, 4.0f), Maths::rand(0.0f, 4.0f));
        float scale = 8.0f * Maths::rand(0.5f, 0.9f) * cloudScale;
        Entity* entity =
            new Entity(Geometry::cube, position, cloudColor, glm::vec3(scale), 1.0f, false, false);
    ...
    glm::vec3 cloudPos(glm::cos(angle) * height,
                       glm::sin(angle) * height - SEA::RADIUS,
                       Maths::rand(-320.0f, -120.0f));
